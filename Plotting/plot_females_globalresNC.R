...
globalresNC <- read.table(paste0(dir,"results/FEMALES.GLOBALRES_NC.ALL.txt"),header=T)
globalresNC$trait <- gsub(".globalresNC.txt","",globalresNC$trait)
globalresNC$trait <- gsub("[[:space:]]+$", "",globalresNC$trait)

#Merge 
globalresNC <- merge(globalresNC,females,by.x="trait",by.y="File")
...
write.table(sig,paste0(dir,"results/FEMALES_GLOBALRES_NC_FDR_survive.txt"),quote=F,col.names=T,row.names=F)
to_label <- c()
...
#Plotting 
png(paste0(dir,"plots/FEMALES_GLOBALRES_NC.png"),width=13,height=7.5,units="in",res=1000)
