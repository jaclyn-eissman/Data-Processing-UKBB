...
cogresNC <- read.table(paste0(dir,"results/MALES.COGRES_NC.ALL.txt"),header=T)
cogresNC$trait <- gsub(".cogresNC.txt","",cogresNC$trait)
cogresNC$trait <- gsub("[[:space:]]+$", "",cogresNC$trait)

#Merge 
cogresNC <- merge(cogresNC,males,by.x="trait",by.y="File")
...
write.table(sig,paste0(dir,"results/MALES_COGRES_NC_FDR_survive.txt"),quote=F,col.names=T,row.names=F)
to_label <- c()
...
#Plotting 
png(paste0(dir,"plots/MALES_COGRES_NC.png"),width=13,height=7.5,units="in",res=1000)
